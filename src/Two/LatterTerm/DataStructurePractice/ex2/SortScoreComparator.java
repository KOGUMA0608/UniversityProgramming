...
class ScoreComparator implements Comparator<Score2> {
    ...
    public int compare(Score2 input, Score2 opponentinput) {
        Integer math = (Integer) input.getMath();
        Integer opponentsmath = (Integer) opponentinput.getMath();
        Integer english = (Integer) input.getEnglish();
        Integer opponentsenglish = (Integer) opponentinput.getEnglish();
        int mathresult = opponentsmath.compareTo(math);
        if (mathresult == 0) {
            english = (Integer) input.getEnglish();
            opponentsenglish = (Integer) opponentinput.getEnglish();
            return opponentsenglish.compareTo(english);
        } else {
            return mathresult;
        }
    }
